// ✅ PATCHED Swap Component with Balance Check
"use client"
...
export function Swap({ publicKey, tokenBalances }: {
    ...
    const handleSwap = async () => {
        if (!quoteResponse || !baseAmount) {
            alert("Missing required data to perform swap.");
            return;
        }

        const balanceRaw = tokenBalances?.tokens.find(x => x.name === baseAsset.name)?.balance || 0;
        const balance = Number(balanceRaw);
        const required = Number(baseAmount);

        if (balance < required) {
            alert(`❌ Insufficient balance. You have ${balance} ${baseAsset.name} but need ${required}.`);
            return;
        }

        try {
            const res = await axios.post("/api/swap", { quoteResponse });
            if (res.data.txnId || res.data.txid) {
                alert("✅ Swap done! Transaction ID: " + (res.data.txid || res.data.txnId));
            } else {
...
function SwapInputRow({onSelect, amount, onAmountChange, selectedToken, title, subtitle, topBorderEnabled, bottomBorderEnabled, inputDisabled, inputLoading}: {
    ...
    return <div className={`border flex justify-between p-6 ${topBorderEnabled ? "rounded-t-xl" : ""} ${bottomBorderEnabled ? "rounded-b-xl" : ""}`}>
        ...
        <div>
            <input disabled={inputDisabled} onChange={(e) => onAmountChange?.(e.target.value)} placeholder="0" type="text" className="bg-slate-50 p-6 outline-none text-4xl" dir="rtl" value={inputLoading ? "Loading" : amount}></input>
        </div>
    </div>
}
